...
def message(payload):
	...
	if BOT_ID != user_id:
		...
		elif text.lower() == 'trump':
			# The name of the file you're going to upload
			file_name = "C:\\Python_Scripts\\Theodosius_SlackAPP\\trump.gif"
			# Call the files.upload method using the WebClient
			# Uploading files requires the `files:write` scope
			client.files_upload(channels=channel_id,initial_comment="SO BE IT",file=file_name,)
		elif text.lower() == 'veto':
			# The name of the file you're going to upload
			file_name = "C:\\Python_Scripts\\Theodosius_SlackAPP\\veto.gif"
			# Call the files.upload method using the WebClient
			# Uploading files requires the `files:write` scope
			client.files_upload(channels=channel_id,initial_comment="Nihil",file=file_name,)
			random_movie = random.choice(notwatched_movie_list)
			poster_url = get_poster(random_movie)
			movie_text = random_movie + '\n' + poster_url
			client.chat_postMessage(channel=channel_id, text=movie_text)
		elif text.lower() == 'call me by your name':
			client.chat_postMessage(channel=channel_id, text='yeah, fuck off')
		else:
			print('No comment')
